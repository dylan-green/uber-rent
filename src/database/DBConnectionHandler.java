...
public class DBConnectionHandler {
    ...
    private void checkBranchExists(String location) throws SQLException {
        PreparedStatement stmt = connection
                .prepareStatement("SELECT * FROM branch WHERE branch.B_LOCATION = ?");
        stmt.setString(1, location);

        ResultSet rs = stmt.executeQuery();

        if (!rs.next()) {
            throw new SQLException("This branch does not exist\n");
        }
    }
}